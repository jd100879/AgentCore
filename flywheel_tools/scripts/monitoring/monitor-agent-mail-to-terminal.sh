...
# Case-insensitive: identity files may use different capitalization than tmux session names.
# When a case mismatch is found, MY_PANE is updated to tmux's authoritative casing.
verify_pane() {
    local actual
    actual=$(tmux display-message -t "$MY_PANE" -p "#{session_name}:#{window_index}.#{pane_index}" 2>/dev/null)

    # Case-insensitive match: handles session name capitalization differences
    if [[ "${actual,,}" = "${MY_PANE,,}" ]] && [ -n "$actual" ]; then
        # Correct MY_PANE to tmux's authoritative casing for future -t targets
        MY_PANE="$actual"
        return 0
    fi

    # Fallback: scan all panes for a case-insensitive match.
    # Handles the case where tmux couldn't find the session due to wrong capitalization.
    local my_pane_lower="${MY_PANE,,}"
    local found_pane
    found_pane=$(tmux list-panes -a -F "#{session_name}:#{window_index}.#{pane_index}" 2>/dev/null | \
        while IFS= read -r p; do
            [[ "${p,,}" = "$my_pane_lower" ]] && echo "$p" && break
        done)
